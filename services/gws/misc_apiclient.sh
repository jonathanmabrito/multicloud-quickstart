...
kubectl exec $GAPOD --namespace="gauth" -- bash -c "curl -s http://gauth-auth/auth/v3/ops/clients -u $CREDS" | jq .data[].client_id || true
...
if [ "$ACT" == "show" ]; then
    if [ "$CLN" == "all" ]; then
        ...
        kubectl exec $GAPOD --namespace="gauth" -- bash -c "curl -s http://gauth-auth/auth/v3/ops/clients -u $CREDS" | tee RSP
        ...
        kubectl exec $GAPOD --namespace="gauth" -- bash -c "curl -s http://gauth-auth/auth/v3/ops/clients/$CLN -u $CREDS" | tee RSP
...
EOF
    ...
    for cl in ${CLIENTS[*]};do
        ...
        kubectl exec $GAPOD --namespace="gauth" -- bash -c "curl -s -XPOST http://gauth-auth/auth/v3/ops/clients -u $CREDS -H 'Content-Type: application/json' -d '$(NEW_API_CLIENT $cl)'" | tee RSP
        sleep 5
...
if [ "$ACT" == "delete" ]; then
    for cl in ${CLIENTS[*]};do
        ...
        kubectl exec $GAPOD --namespace="gauth" -- bash -c "curl -s -XDELETE http://gauth-auth/auth/v3/ops/clients/$cl -u $CREDS" | tee RSP
        echo;echo "________________________________________________________________________________"
    done
fi
...
EOF
    ...
    for cl in ${CLIENTS[*]};do
        ...
        kubectl exec $GAPOD --namespace="gauth" -- bash -c "curl -s -XPUT http://gauth-auth/auth/v3/ops/clients/$cl -u $CREDS -H 'Content-Type: application/json' -d '$(NEW_REDURI)'" | tee RSP
        echo;echo "________________________________________________________________________________"
    done
fi
...
kubectl exec $GAPOD --namespace="gauth" -- bash -c "curl -s http://gauth-auth/auth/v3/ops/clients -u $CREDS" | jq .data[].client_id